...
class NewTopicTests(TestCase):
    ...
    def test_csrf(self):        #since the CSRF Token is a fundamental part of processing POST requests, we have to make sure our HTML contains the token
        url = reverse('new_topic', kwargs={'pk': 1})
        response = self.client.get(url)
    ...
    def test_new_topic_invalid_post_data(self):     #here we are sending an empty dictionary to check how the application is behaving
        '''
    ...
    def test_new_topic_invalid_post_data_empty_fields(self):        #similar to the previous test, but this time we are sending some data. The application is expected to validate and reject empty subject and message.
        '''
        Invalid post data should not redirect
        The expected behavior is to show the form again with validation errors
        '''
